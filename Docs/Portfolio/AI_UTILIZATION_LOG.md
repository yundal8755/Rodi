...
| 상태 | 후보 | 강조할 기술 포인트 | 확정에 필요한 근거 |
| --- | --- | --- | --- |
| 회고 필요 | 후기 작성·수정 flow | 다단계 form, child reducer, 편집 데이터 prefill, 완료 뒤 목록 refresh | 작성·수정·취소·실패 수동 QA 결과 |
| 회고 필요 | 코스 상세 후기 | level cache, cursor pagination, 신고·차단 후 세션 내 즉시 숨김 | level 전환·페이지네이션·늦은 응답 QA |
| 회고 필요 | 코스 등록 | 지도 선택, 경유지 편집, 장소 검색, route 생성 | 출발/경유/도착·삭제·검색·제출 QA |
| 회고 필요 | 연습 추적·Live Activity | 위치 측정 session, 앱 복귀, Live Activity 상태 동기화 | 권한·background/foreground·종료 시나리오 |
...
- Architecture Layer 문서의 MUST/MUST NOT과 Feature foldering 기준
- Figma·Swagger·Release 문서의 작업별 원본 분리
- Presentation 리팩터링 백로그처럼 규칙 원본과 변경 가능한 작업 상태를 분리한 사례
...
- MUST 테스트 target이 없으면 자동 테스트 통과라고 기록하지 않는다.
