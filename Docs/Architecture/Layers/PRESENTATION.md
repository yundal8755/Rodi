...
- MUST `PracticeTracking` root에는 `PracticeTrackingView`와 `PracticeTrackingReducer`만 두고, 앱 복귀 정책은 `Return`, lifecycle bridge는 `Adapter`, 측정·저장 구현은 `Service`, Live Activity runtime은 `LiveActivity`에 둔다.
